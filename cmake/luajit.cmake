...
macro(luajit_build)
    ...
    if (${CMAKE_SYSTEM_NAME} STREQUAL Darwin)
        ...
        if ("${CMAKE_OSX_DEPLOYMENT_TARGET}" STREQUAL "")
            ...
            string(REGEX MATCH "([0-9]+.[0-9]+).[0-9]+" DARWIN_VERSION ${PRODUCT_VERSION})
            message(STATUS "DARWIN_VERSION=${DARWIN_VERSION}")
            string(REGEX MATCH "([0-9]+).[0-9]+" MAJOR_VERSION ${DARWIN_VERSION})
            message(STATUS "MAJOR_VERSION=${MAJOR_VERSION}")
            if (MAJOR_VERSION GREATER 9)
                set (luajit_osx_deployment_target 10.14)
            elseif (MAJOR_VERSION LESS 9)
                set (luajit_osx_deployment_target 10.6)
            elseif (MAJOR_VERSION EQUAL 9)
                string(REGEX MATCH "[0-9]+.([0-9]+)" MINOR_VERSION ${DARWIN_VERSION})
                message(STATUS "MINOR_VERSION=${MINOR_VERSION}")
                if (MINOR_VERSION GREATER 3)
                    set (luajit_osx_deployment_target 10.14)
                else ()
                    set (luajit_osx_deployment_target 10.6)
                endif ()
